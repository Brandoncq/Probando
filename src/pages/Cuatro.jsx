...
function Cuatro() {
  ...
  const resolver = () => {
    const { x, a, c, m, repeticiones, confianza } = variables;

    // Verificar que todas las variables existan y no estén vacías
    if (x && a && c && m && repeticiones && confianza) {
      let i = 0;
      let xValues = [];
      let rValues = [];
      ...
      for (i = 1; i < Number(repeticiones); i++) {
        const data =
          (Number(variables.c) + Number(variables.a) * xValues[i - 1]) %
          Number(variables.m);
        xValues.push(data);
