...
function Nueve() {
  ...
  const resolver = () => {
    const { x, a, repeticiones, m, confianza } = variables;

    // Verificar que todas las variables existan y no estén vacías
    if (x && a && repeticiones && m && confianza) {
      let xValues = [];
      let rValues = [];
      ...
      for (let i = 1; i < Number(repeticiones); i++) {
        const data =
          (Number(variables.a) * xValues[i - 1]) % Number(variables.m);
            ...
            <table className="w-full text-sm text-left rtl:text-right text-gray-200">
              ...
              <tbody>
                ...
                <tr className="border-b bg-zinc-800 border-gray-700 hover:bg-gray-600">
                  ...
                  <td className="px-6 py-4">Repetiones</td>
                  <td className="px-6 py-4">
                    <input
                      className="bg-transparent w-full outline-none"
                      type="number"
